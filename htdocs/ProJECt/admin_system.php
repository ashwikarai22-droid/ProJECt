...
");
       ...
       <b>Mentor:</b> <?= $row['mentor_name'] ?? 'Not Assigned' ?>
                        ...
                        <td>
<?= htmlspecialchars($row['leader_name']) ?><br><br>

<button class="btn btn-view"
onclick="toggleBlock('members<?= $row['project_id'] ?>')">
View Members
</button>

<div id="members<?= $row['project_id'] ?>" 
     style="display:none;margin-top:10px;background:#f8fafc;padding:10px;border-radius:6px;">

<b>Leader</b><br>
<?= $row['leader_name'] ?> (<?= $row['leader_roll_no'] ?>)<br>
<?= $row['leader_branch'] ?> | Sem <?= $row['leader_semester'] ?><br>
...
for ($i = 1; $i <= 3; $i++) {
    if (!empty($row["member{$i}_name"])) {
        echo "
        <div style='margin-top:6px;padding-left:8px;border-left:3px solid #3498db'>
            {$row["member{$i}_name"]} ({$row["member{$i}_roll_no"]})<br>
            {$row["member{$i}_branch"]} | Sem {$row["member{$i}_sem"]}<br>
...
?>
                        ...
                        <td><?= $row['leader_email'] ?><br><?= $row['leader_phone'] ?></td>
                        <td>
<button class="btn btn-view"
onclick="toggleBlock('idea<?= $row['project_id'] ?>')">
View Submitted Idea
</button>

<div id="idea<?= $row['project_id'] ?>"
     style="display:none;margin-top:10px;background:#f8fafc;padding:10px;border-radius:6px;">
...
<button class="btn btn-cyan">Open File</button>
</a>
<?php else: ?>
