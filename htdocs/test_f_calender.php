...
/* SAVE MEETING */
if (isset($_POST['save_meeting']) && $canEdit) {
    $title = $conn->real_escape_string($_POST['title']);
    $date = $_POST['date'];
    $time = $_POST['meeting_time'];
    $link = $conn->real_escape_string($_POST['meeting_link']);

    $conn->query("INSERT INTO calendar_events 
    (title, event_date, event_type, meeting_time, meeting_link)
    VALUES ('$title', '$date', 'meeting', '$time', '$link')");
...
<div id="list"></div>
<button onclick="openMeetingForm()">➕ Schedule Meeting</button>
...
<form method="POST" id="meetingForm" style="display:none;">
...
<input name="meeting_link" placeholder="Meeting Link (Google Meet / Zoom)" required>

<input type="hidden" name="event_type" value="meeting">

<button name="save_meeting">Schedule Meeting</button>
</form>
</form>
<?php endif; ?>
</div>
...
function load(ds){
    ...
    events.filter(e => e.event_date === ds).forEach(e => {
        ...
        <div class="event">
            ...
            <a href="?delete=${e.id}" onclick="return confirm('Delete?')">🗑</a>
        </div>`;
    });
}  
...
function openMeetingForm(){
    ...
    document.getElementById('mdate').value = sel;
}
